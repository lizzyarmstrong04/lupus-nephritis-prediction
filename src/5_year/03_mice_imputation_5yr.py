...
df = pd.read_excel(f"{DATA_DIR}/lupus_5yr_flare_dataset.xlsx")
...
holdout_cols = [c for c in [ID_COL, OUTCOME_COL] + date_cols + text_cols if c in df.columns]
...
cols_to_impute   = cols_under_5 + cols_5_to_50
...
for col in before.index:
    print(f"  {before[col]:.1f}% → {after_missing.get(col, 0.0):.1f}%  {col}")

output_path = f"{PROCESSED_DIR}/lupus_5yr_imputed.xlsx"

...
# 5-year cohort: 190 controls / 166 cases → ratio ≈ 1.14 (nearly balanced)
...
for name, pipeline in base_pipelines.items():
    ...
    search = RandomizedSearchCV(
        ...
        n_iter=40, scoring="roc_auc", cv=inner_cv,
...
# Save best_params for SHAP script
with open(f"{OUTPUTS_DIR}/5yr_best_params.json", "w") as f:
    json.dump({k: {pk: (int(pv) if isinstance(pv, np.integer) else
                        float(pv) if isinstance(pv, np.floating) else pv)
                   for pk, pv in v.items()}
               for k, v in best_params.items()}, f, indent=2)
print(f"\nSaved best_params to: {OUTPUTS_DIR}/5yr_best_params.json")

# ============================================================
# Build final model set
# ============================================================
def make_rf(p):
...
# ============================================================
print("\n" + "="*60)
print("STEP 2 — 5×10-fold cross-validation")
print("="*60)
...
ax_roc.set_title("ROC Curves — 5-Year Lupus Nephritis Flare\n(tuned models)", fontsize=12)
ax_roc.legend(loc="lower right", fontsize=8.5)
ax_roc.set_xlim([0, 1]); ax_roc.set_ylim([0, 1])
plt.tight_layout()
fig_roc.savefig(f"{FIGURES_DIR}/roc_curves_5yr.png", dpi=200, bbox_inches="tight")
plt.close(fig_roc)
print("  Saved: roc_curves_5yr.png")
...
ax_cal.set_title("Calibration Curves — 5-Year Lupus Nephritis Flare\n(OOF from 5×10-fold CV)", fontsize=12)
ax_cal.legend(loc="upper left", fontsize=8.5)
ax_cal.set_xlim([0, 1]); ax_cal.set_ylim([0, 1])
plt.tight_layout()
fig_cal.savefig(f"{FIGURES_DIR}/calibration_curves_5yr.png", dpi=200, bbox_inches="tight")
plt.close(fig_cal)
print("  Saved: calibration_curves_5yr.png")
...
output_path = f"{OUTPUTS_DIR}/5yr_model_results.xlsx"
...
print(f"  Saved: 5yr_model_results.xlsx")

...
# --- 2b. Named leakage columns (post-baseline measurements) ---
named_leakage = [
    ...
    # Change from baseline to 1yr (requires knowing 1yr value)
    ...
    # Flare definition column (is the outcome itself)
    ...
    # Latest follow-up creatinine and derived columns
    ...
    "4=Death 5=Death on RRT 6=eGFR<80 but not doubling creatinine, RRT or death, 7=eGFR<70 but "
    "not doubling creatinine, RRT or death 8=eGFR<60 but not doubling creatinine, RRT ",
    # Additional outcome columns with amended codes
...
# --- 2c. Keyword catch-all for any remaining leakage ---
LEAKAGE_KEYWORDS = [
...
# --- Build baseline-only feature matrix ---
...
# Drop columns with remaining missing (>50% missing, excluded from MICE)
incomplete = X.columns[X.isnull().any()].tolist()
X = X.drop(columns=incomplete)
print(f"\nAlso dropped {len(incomplete)} columns still containing missing values (>50% originally missing)")
...
for c in binary_cols:
    ...
    if max_freq > 0.90:
        ...
        removal_log[c] = f"Step 3a — Dominant binary (majority class = {max_freq*100:.1f}%)"
...
while changed:
    ...
    for col in upper.columns:
        ...
        if partners:
            ...
            removal_log[col] = f"Step 4 — High correlation (r={r} with '{partners[0]}', kept '{partners[0]}')"
...
if X.shape[1] > EPV_MAX:
    # Search over decreasing C values (increasing regularisation)
    ...
    print(f"LASSO regularisation strength used: C={chosen_C}")
